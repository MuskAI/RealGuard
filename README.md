...
```
./scripts/train.sh --data_path /raid0/chr/AIGCD/Datasets/CNNSpot_trainingdata/progan_train --eval_data_path /raid0/chr/AIGCD/Datasets/CNNSpot_trainingdata/progan_train --resnet_path pretrained_ckpts/resnet50-19c8e357.pth --convnext_path /raid5/chr/AIGCD/download/CLIP-convnext_xxlarge-laion2B-s34B-b82K-augreg-soup/open_clip_pytorch_model.bin --output_dir results/progan_train
```

# Improvement
1. 根本没有必要用ConvNeXt-XXLarge的版本太大了，首先要缩小模型大小，直接换成Base
2. 还是先train第一个分支比较好
3. patch的大小是256，因为我们是为了高分辨率服务的
4. 使用bucket sampling
5. 采取分阶段训练的模式
