...
from data.datasets_mil import TrainDataset, TestDataset
# from engine_finetune import train_one_epoch, evaluate
# added by haoran
from engine_finetune_mil import train_one_epoch, evaluate
######################################
import utils
...
from mil_loss import PatchAwareCELoss
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='/raid5/chr/AIGCD/AIDE/results/MIL-V1',
...
def main(args):
    ...
    dataset_train = TrainDataset(is_train=True, args=args) # 还
    ...
    if args.eval:
        ...
        for v_id, val in enumerate(vals):
            ...
            test_stats, acc, ap = evaluate(data_loader_val, model, device,criterion)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            test_stats, acc, ap = evaluate(data_loader_val, model, device, use_amp=args.use_amp,criterion=criterion)
            ...
            if args.model_ema and args.model_ema_eval:
                test_stats_ema, acc, ap = evaluate(data_loader_val, model_ema.ema, device, use_amp=args.use_amp,criterion=criterion)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('PMIL traning', parents=[get_args_parser()])
