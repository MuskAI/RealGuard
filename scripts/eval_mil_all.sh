...
for ((i=1; i<=1; i++))
do
    CKPT=results/cnnspot-sd14-mil_fix-res50-rgb_pure_milloss/checkpoint-$i.pth
    if [ -f "$CKPT" ]; then
        ...
        CUDA_VISIBLE_DEVICES=0 python eval_all.py \
            ...
            --resnet_path "$CKPT" \
            --output_dir /raid5/chr/AIGCD/AIDE/eval_results/cnnspot-sd14-mil_fix-res50-rgb_pure_milloss/ \
            --csv_file_name cnnspot-sd14-mil_fix-res50-rgb_pure_milloss-e$i \
            --data_mode mil &
